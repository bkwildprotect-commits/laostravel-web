...
export async function applyBookingRewards(tx:TransactionContext,repo:RewardTransactionRepository,input:{userId:string;bookingId:string;pointsToRedeem?:number;couponCode?:string;couponBenefitAmount?:string}){
 ...
 if(points<0n)throw new Error("INVALID_POINTS");
 if(points>0n){await repo.lockUserPoints(tx,input.userId);const available=await repo.getAvailablePoints(tx,input.userId);if(available<points)throw new Error("INSUFFICIENT_POINTS");await repo.redeemPoints(tx,{userId:input.userId,bookingId:input.bookingId,points});}
