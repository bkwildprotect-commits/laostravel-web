...
CREATE TABLE availability (id uuid PRIMARY KEY, service_id uuid NOT NULL REFERENCES services(id), starts_at timestamptz, ends_at timestamptz, capacity integer, remaining integer, version bigint NOT NULL DEFAULT 1, CHECK (remaining IS NULL OR remaining >= 0), UNIQUE(id,service_id));
